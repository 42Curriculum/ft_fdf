...
/*   Updated: 2019/11/19 03:47:36 by jjosephi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**error_loop(char *arg, int *len)
{
	...
	{
		ft_putstr("\e[0;32m Input a file or type !edit \n");
		b = read(0, input, 209);
		input[b - 1] = '\0';
		if (!(ft_strcmp("!edit", input)))
		{
			if (error_code == -1)
				ft_putstr("\e[0;32m Cannot edit directory or invalid file\n");
			else
			{
				edit_file(arg);
				input = ft_strcpy(input, arg);
			}
		}
	}
	free(input);
